...
void sigma_order(fq_nmod_poly_t res, const fq_nmod_t x, const fq_nmod_ctx_t field) {
	
	// We treat the degenerate case x = 0
	if (fq_nmod_is_zero(x, field)) {
		fq_nmod_poly_one(res, field);
	}
		...
		for (i = 0; i < xcopy->length; i++) {
			fq_nmod_set_fmpz(tmp, xcopy->coeffs + i, field);
		...
		fq_nmod_poly_neg(P, P, field);
		fq_nmod_one(tmp, field);
		fq_nmod_poly_set_coeff(P, k-1, tmp, field); 

		// We set res to P (our result)
		fq_nmod_poly_set(res, P, field);

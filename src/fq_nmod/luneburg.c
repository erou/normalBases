...
void luneburg(fq_nmod_t res, const fq_nmod_ctx_t field) {
	...
	fq_nmod_t X, a, tmp, rop;
	fq_nmod_init(X, field);
	fq_nmod_init(a, field);
	fq_nmod_init(tmp, field);
	fq_nmod_init(rop, field);
	...
	// f_i and g_j
	fq_nmod_poly_factor_t f, g;
	fq_nmod_poly_factor_init(f, field);	
	fq_nmod_poly_factor_init(g, field);	

	// We initialise the polynomials h and P (a temporary polynomial)
	fq_nmod_poly_t P, h;
	fq_nmod_poly_init(P, field);
	fq_nmod_poly_init(h, field);

    // We create a list of polynomials
	fq_nmod_poly_struct *F;
	F = (fq_nmod_poly_struct*)malloc(d*sizeof(fq_nmod_poly_struct));

	// We create f_0, and we insert it in f
	fq_nmod_poly_init(F, field);
	fq_nmod_one(a, field);
	sigma_order(F, a, field);
	fq_nmod_poly_factor_insert(f, F, 1, field);


	// And we do the same for f_i (i = 1 .. d-1)
	for (i = 1; i < d; i++) {
		fq_nmod_poly_init(F + i, field);
		fq_nmod_mul(a, a, X, field);
		sigma_order(F + i, a, field);
		fq_nmod_poly_factor_insert(f, F + i, 1, field);
	}
	...
	for (j = 0; j < n; j++) {
		...
		// And we compute β_j 
		fq_nmod_poly_pow(P, g->poly + j, expmax, field);
		fq_nmod_poly_divides(h, F + ind, P, field);
		fq_nmod_pow_ui(tmp, X, ind, field);
		frobenius_composition(tmp, h, tmp, field);

		// Then we add it to rop
		fq_nmod_add(rop, rop, tmp, field);
	}

	fq_nmod_set(res, rop, field);

	// Then we clear all the variables
	fq_nmod_clear(X, field);
	fq_nmod_clear(a, field);
	fq_nmod_clear(tmp, field);
	fq_nmod_clear(rop, field);
	fq_nmod_poly_clear(P, field);
	fq_nmod_poly_clear(h, field);
	fq_nmod_poly_factor_clear(f, field);
	fq_nmod_poly_factor_clear(g, field);
    free(F);
}
